...
class _PieChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < pieValues.length; i++) {
      ...
      drawPieArc(pieValues[index],gap > 0.0 && index%2 !=0 ?  Colors.transparent : pies[index~/(gap == 0.0 ? 1 : 2) ].color , size, canvas);
      ...
      showPieText(pies[index~/(gap == 0.0 ? 1 : 2)].value, size.width/2, size, canvas);
      }
  ...
  drawCenterText(Canvas canvas, Size size){
     final textSpan = TextSpan(
          text: centerText,
          style:
              centerStyle ?? const TextStyle(color: Colors.black, fontSize: 20.0));
      final textPainter =
